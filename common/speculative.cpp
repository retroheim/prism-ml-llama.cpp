...
// Suffix-tree self-speculative decoding (Saxena et al. 2024 — arXiv:2411.04975).
// Stores all suffixes of the rolling token history in a trie and uses it to
// guess upcoming tokens whenever the current context end matches a recurring
// pattern. No draft model required.
struct common_speculative_state_suffix : public common_speculative_state {
    ...
    // Adaptive p_min state: tighten when we keep generating low-acceptance drafts.
    size_t n_draft_last = 0;
        ...
        , corpus_tree(max_depth)
    {
        if (!corpus_path.empty()) {
            ...
            if (model) {
                ...
                tokenize_fn = [vocab](const std::string & text) -> std::vector<llama_token> {
                    return common_tokenize(vocab, text, false, true);
                };
            }
    ...
    void draft(
        ...
        const int min_match_len = std::max(1, sp.min_match_len);
    ...
    for (const common_speculative_config & config : configs) {
        ...
        switch (config.type) {
            ...
            case COMMON_SPECULATIVE_TYPE_SUFFIX: {
                const int depth = config.params.suffix.max_depth > 0 ? config.params.suffix.max_depth : 64;
                impls.push_back(std::make_unique<common_speculative_state_suffix>(
                    config.type,
                    depth,
                    config.params.suffix.corpus_path,
                    llama_get_model(ctx_tgt)));
                break;
            }
            default:
                break;
